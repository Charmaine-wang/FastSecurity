<?php

require __DIR__.'/header.php';

 ?>

<section class="leave-email">

  <div class="contactsheet-background">
    <img src="./images/contactsheet-background.png" alt="">
  </div>

  <div class="download">
    <img class="saab-big" src="./images/saablogo.png" alt="Logo">
    <img class="fs-logo2" src="./images/FASTSECURITY_FS_LOGO.png" alt="Logo">
    <h2>Continue <br>learning more!</h2>
    <p>Thank you for registrating.
      <br>Download our brochure below to
      <br>learn more about Fast Security
    </p>
    <button class="download-button" type="submit">Download your brochure here
      <i class="fas fa-download"></i>
    </button>
    <hr class="hr-download">
    <a href="#">
      <button class="back-button" type="button" name="button">
        <i class="fas fa-angle-left"></i> Take me back
      </button>
      </a>
  </div>

</section>
